...
def generate_markdown(
    ...
    :return: 文件生成结果说明
    """
    logger.debug("输入保存路径", path=path or "当前会话目录")
    ...
    logger.debug("当前会话目录", session_dir=session_dir)
    ...
    logger.debug("路径解析", parent_dir=str(parent_dir), filename=filename, full_path=str(file_path))

    try:
        # 允许模型指定 session_dir 下的子目录；不存在时自动创建
        if not parent_dir.exists():
            parent_dir.mkdir(parents=True, exist_ok=True)
            logger.debug("已创建目录", parent_dir=str(parent_dir))

        file_path.write_text(content, encoding="utf-8")

        logger.info("文件写入完成", file_path=str(file_path))
        return f"Markdown文件 '{file_path}' 已成功生成并保存。"
    except Exception as e:
        logger.error("文件写入失败", exc_info=True)
